 ...
 */
var _a, _b, _c;
import { BackSide, BoxBufferGeometry, Cache, CubeCamera, EventDispatcher, GammaEncoding, LinearToneMapping, Mesh, NearestFilter, RGBEEncoding, RGBEFormat, Scene, ShaderMaterial, TextureLoader, UnsignedByteType } from 'three';
...
const $loadEnvironmentMapFromSkyboxUrl = Symbol('loadEnvironmentMapFromSkyboxUrl');
...
export default class TextureUtils extends EventDispatcher {
     ...
     */
    [(_a = $generatedEnvironmentMap, _b = $environmentMapCache, _c = $skyboxCache, $loadSkyboxFromUrl)](url, progressTracker) {
        if (!this[$skyboxCache].has(url)) {
            const progressCallback = progressTracker ? progressTracker.beginActivity() : () => { };
            this[$skyboxCache].set(url, this.loadEquirectAsCubeMap(url, progressCallback));
        }
    ...
    [$loadEnvironmentMapFromUrl](url, progressTracker) {
        if (!this[$environmentMapCache].has(url)) {
            ...
            const environmentMapLoads = this.loadEquirectAsCubeMap(url, progressCallback)
                ...
                const environmentMap = this.pmremPass(interstitialEnvironmentMap.texture);
     ...
     */
    [$loadEnvironmentMapFromSkyboxUrl](url, progressTracker) {
        if (!this[$environmentMapCache].has(url)) {
            const skyboxLoads = this[$loadSkyboxFromUrl](url, progressTracker);
            const environmentMapLoads = skyboxLoads.then(skybox => this.pmremPass(skybox.texture));
            this[$environmentMapCache].set(url, environmentMapLoads);
        }
        return this[$environmentMapCache].get(url);
    }
    ...
    gaussianBlur(cubeTarget, standardDeviationRadians = 0.04) {
        const blurScene = new Scene();
        const geometry = new BoxBufferGeometry();
        geometry.removeAttribute('uv');
        const cubeResolution = cubeTarget.width;
        const standardDeviations = 3;
        const n = Math.ceil(standardDeviations * standardDeviationRadians * cubeResolution * 2 /
            Math.PI);
